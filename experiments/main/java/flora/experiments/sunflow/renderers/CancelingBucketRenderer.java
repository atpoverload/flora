...
/** A copy of the {@link BucketRenderer} that will stop rendering if a thread throws. */
// TODO: must be public for Sunflow to be able to construct this
public final class CancelingBucketRenderer implements ImageSampler {
  private Scene scene;
  ...
  public CancelingBucketRenderer() {
    bucketSize = 32;
  ...
  public boolean prepare(Options options, Scene scene, int w, int h) {
    ...
    if (minAADepth == maxAADepth) maxStepSize = minStepSize;
    else maxStepSize = minAADepth > 0 ? 1 << minAADepth : subPixelSize << (-minAADepth);
    ...
    if (filter == null) {
      UI.printWarning(
          Module.BCKT, "Unrecognized filter type: \"%s\" - defaulting to box", filterName);
    ...
    sigmaOrder =
        Math.min(
            QMC.MAX_SIGMA_ORDER,
            Math.max(0, maxAADepth) + 13); // FIXME: how big should the table be?
    ...
    UI.printInfo(
        Module.BCKT,
        "  * Subpixel jitter:    %s",
        useJitter ? "on" : (jitter ? "auto-off" : "off"));
  ...
  private String aaDepthToString(int depth) {
    ...
    return String.format(
        "%s%d sample%s", depth < 0 ? "1/" : "", pixelAA * pixelAA, depth == 0 ? "" : "s");
  }
    ...
    public void run() {
      while (true) {
        ...
        try {
          ...
          if (UI.taskCanceled()) {
            UI.printError(Module.BCKT, "Canceling due to out of memory while rendering bucket");
            UI.taskCancel();
  ...
  private void refineSamples(
    ...
    if (!s00.sampled()) computeSubPixel(s00, istate);
    if (!s01.sampled()) computeSubPixel(s01, istate);
    if (!s10.sampled()) computeSubPixel(s10, istate);
    if (!s11.sampled()) computeSubPixel(s11, istate);
  ...
  private static final class ImageSample {
    ...
    final boolean isDifferent(ImageSample sample, float thresh) {
      if (instance != sample.instance) return true;
      if (shader != sample.shader) return true;
      if (Color.hasContrast(c, sample.c, thresh)) return true;
      if (Math.abs(alpha - sample.alpha) / (alpha + sample.alpha) > thresh) return true;
      // only compare normals if this pixel has not been averaged
      float dot = (nx * sample.nx + ny * sample.ny + nz * sample.nz);
      return dot < 0.9f;
    }

    static final ImageSample bilerp(
        ImageSample result,
        ImageSample i00,
        ImageSample i01,
        ImageSample i10,
