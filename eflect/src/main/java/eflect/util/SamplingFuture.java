...
public final class SamplingFuture<T> implements Future<List<T>> {
  ...
  // TODO: we can make this support more complex schedules
  ...
  private SamplingFuture(
      Supplier<? extends T> source, Supplier<Duration> period, ScheduledExecutorService executor) {
    this.period = period;
    this.executor = executor;
    synchronized (dataFutures) {
      // TODO: if the source throws an exception, it just get propagated to the caller of get
  ...
  private Optional<? extends T> collectDataAndReschedule(Supplier<? extends T> source) {
    ...
    T data = source.get();
    Duration rescheduleTime = period.get().minus(Duration.between(start, Instant.now()));
