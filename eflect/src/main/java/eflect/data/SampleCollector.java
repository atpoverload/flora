...
public class SampleCollector<O> {
  ...
  private void stopFutures() {
    synchronized (futures) {
      getLogger().fine(String.format("futures total %d", futures.size()));
      ...
      for (Future<?> future : futures) {
        ...
        if (!future.isDone() && !future.isCancelled() && !future.cancel(false)) {
          try {
            ...
            getLogger().log(Level.FINE, "could not consume a future", e);
            e.printStackTrace();
          }
        }
      }
  ...
  private void runAndReschedule(Runnable r) {
    ...
    if (rescheduleTime.toNanos() > 0) {
      ...
      getLogger().fine(String.format("lagging behind by %d", rescheduleTime.abs().toMillis()));
